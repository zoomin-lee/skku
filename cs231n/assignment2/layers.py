...
def affine_backward(dout, cache):
    ...
    # input gradient [10,5]*[5,6]=[10,6]->[10,2,3]
    dx = dout.dot(w.T)
    dx = dx.reshape(x.shape)

    # weight gradient [6,10]*[10,5]=[6,5]
    dw = X.T.dot(dout)

    # bias gradient [10,5]->[5]
    db = dout.sum(axis=0)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        mu = x.mean(axis=0)       #[D,]
        var = x.var(axis=0) + eps #[D,]
        std = np.sqrt(var)        #[D,]
        z = (x - mu)/std          #[N,D]
        out = gamma * z + beta    #[N,D]
...
def batchnorm_backward(dout, cache):
    ...
    dxhat = dout * cache['gamma']  #[N,D]
    dx1 = dxhat / cache['std']     #[N,D]

    dvar = np.sum(dxhat * (-0.5) * (cache['x'] - cache['mean']) * (cache['var']) ** (-1.5), axis=0) #[D,]
    dx2 = dvar * 2 * (cache['x'] - cache['mean']) / m                                               #[N,D]

    dmu = np.sum(dxhat / (-cache['std']), axis=0)  #[D,]
    dx3 = dmu / m                                  #[N,D]
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxhat = dout * cache['gamma']                                               #[NxD]
    dxhat_sum = np.sum(dxhat,axis=0)                                            #[1xD]
    dx = dxhat - dxhat_sum/m - np.sum(dxhat * cache['z'],axis=0) * cache['z']/m #[NxD]
    dx /= cache['std']
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    act_H = int(1 + (H - HH + 2 * pad) / stride)
    act_W = int(1 + (W - WW + 2 * pad) / stride)
    out = np.zeros((N, F, act_H, act_W))

    # bb(2차원)의 상하좌우 모두 2개 행/열 0으로 패딩합니다
...
def conv_backward_naive(dout, cache):
    ...
    dx = dx_pad[:, :, pad : Hpad-pad, pad : Wpad-pad]
      ...
      for c in range(C):
        for h in range(H_out):
          for w in range(W_out):
            current_matrix= x[n,c, h*stride:h*stride+pool_h, w*stride:w*stride+pool_w]
            out[n,c,h,w] = np.max(current_matrix)

            #np.unravel_index([1, 2, 8], (3,3)) : 1을 (3,3)좌표에서 표현했을 때 (0,1), 2 =(0,2), 8=(2,2)
            #->(array([0, 0, 2], dtype=int64), array([1, 2, 2], dtype=int64))

            switches[n, c, h, w] = np.unravel_index(current_matrix.argmax(), current_matrix.shape)  
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for h in range(H_out):
          for w in range(W_out):
            local_index = switches[n, c, h, w]
            hmax, wmax = local_index[0] + h*stride,local_index[1] + w*stride
            dx[n, c, hmax, wmax] += dout[n, c, h, w]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    # shape(2, 3, 4) => shape(3, 2, 4) : x.transpose(1,0,2)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    z = cache['z'].reshape(size).T      #[(C/G)*H*W, N*G]
    M = z.shape[0]                      #[(C/G)*H*W]
    dxhat = dout * cache['gamma']       #[N,C,H,W]
    dxhat_T = dxhat.reshape(size).T     #[(C/G)*H*W, N*G]

    dxhat_sum = np.sum(dxhat_T,axis=0)  #[N*G]
    dx = dxhat_T - dxhat_sum/M - np.sum(dxhat_T * z,axis=0) *z/M
    dx /= cache['std']
